...
class MyModel(nn.Module):
    def __init__(self,hidden_size,num_labels,model_name_or_path,model_config,cache_dir):
        ...
        # self.lm_head
        self.lm_head=self.roberta_for_masked_lm.lm_head
        ...
        self.decoder=self.roberta_for_masked_lm.lm_head.decoder
...
class TransformerModelWrapper:
    ...
    def __init__(self, config: WrapperConfig):
        ...
        self.loss_lm_dict={"loss_s": [], "loss_s_with_uncertainty": [], "loss_s_on_vocab_words": [], "loss_p_labeled": [], "loss_p_unlabeled": [], "loss_kl": [], "loss_total": [], "loss_on_labeled": [], "kv_labeled_loss":[], "unlabeled_lm_loss":[], "faraway_loss":[]}
        self.epoch_loss_lm_dict={"loss_s": [], "loss_s_with_uncertainty": [], "loss_s_on_vocab_words": [], "loss_p_labeled": [], "loss_p_unlabeled": [], "loss_kl": [], "loss_total": [], "loss_on_labeled": [], "kv_labeled_loss":[], "unlabeled_lm_loss":[], "faraway_loss":[]}
        ...
        self.masked_lm_model= self.masked_lm_model.to("cuda")
        ...
        for _ in train_iterator: #epochs
            ...
            with torch.no_grad():
                ...
                for _, batch in enumerate(unlabeled_pool_iterator):
                    ...
                    unlabeled_pred_logits=F.softmax(unlabeled_pred_logits,dim=1)  #softmax
                ...
                print("curr epoch uncertainty : ",self.uncertainty)
            ...
            self.cluster_match_dict = cluster_eval(new_classes_samples_gt_labels,new_classes_samples_pred_labels,self.known_classes,self.total_classes)
            ...
            self.write_results(self.total_classes,self.known_classes,self.true_label_pool,self.fnn_cluster_list)     
        
            if curr_tolerance>=max_tolerance:
                self.train_lm_model(task_train_data, device, per_gpu_train_batch_size, n_gpu, num_train_epochs, gradient_accumulation_steps, weight_decay,
                                    learning_rate, adam_epsilon, warmup_steps, max_grad_norm, logging_steps, per_gpu_unlabeled_batch_size,
                                    unlabeled_data, lm_training, use_logits, alpha, temperature, max_steps)
                break
    ...
    def train_lm_model(self, task_train_data: List[InputExample], device, per_gpu_train_batch_size: int = 8, n_gpu: int = 1,
              num_train_epochs: int = 3, gradient_accumulation_steps: int = 1, weight_decay: float = 0.0,
              learning_rate: float = 5e-5, adam_epsilon: float = 1e-8, warmup_steps=0, max_grad_norm: float = 1,
              logging_steps: int = 50, per_gpu_unlabeled_batch_size: int = 8, unlabeled_data: List[InputExample] = None,
              lm_training: bool = False, use_logits: bool = False, alpha: float = 0.8, temperature: float = 1,
        ...
        # unlabeled_data length
        unlabeled_data_length=len(unlabeled_data)
        ...
        no_decay = ['bias', 'LayerNorm.weight']
        optimizer_grouped_parameters = [
            {'params': [p for n, p in self.masked_lm_model.named_parameters() if not any(nd in n for nd in no_decay)],
             'weight_decay': weight_decay},
            {'params': [p for n, p in self.masked_lm_model.named_parameters() if any(nd in n for nd in no_decay)],
             'weight_decay': 0.0}
        ]
        ...
        global_step = 0
        tr_loss, logging_loss = 0.0, 0.0
        self.masked_lm_model.zero_grad()
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(unlabeled_pool_iterator):
                ...
                unlabeled_pred_logits=unlabeled_outputs[0][unlabeled_batch['mlm_labels']>=0] 
                max_idxs=unlabeled_pred_logits.argmax(dim=1).cpu().numpy().tolist()
                self.fnn_cluster_list.extend(max_idxs)
            for i,val in enumerate(self.fnn_cluster_list):
                self.limited_cluster_dict[val].append(i)
        
        self.self_lm_defined_epoch=0
        curr_tolerance=0
        max_tolerance=3
        loss_min= 1000000
        for _ in train_iterator: #epochs
            self.self_lm_defined_epoch+=1
            unlabeled_pool_iterator = tqdm(unlabeled_dataloader, desc="update pool")
            self.vocab_logits_pool= None
            self.true_label_pool=None
            self.top30_for_each_sample=[] 
            ...
            with torch.no_grad():
                ...
                for i_step, batch in enumerate(epoch_iterator):
                    self.masked_lm_model.eval()
                    labeled_batch = {k: t.to(device) for k, t in batch.items()}
                    labeled_inputs=self.generate_default_inputs(labeled_batch)
                    mlm_labels, labels = labeled_batch['mlm_labels'], labeled_batch['labels']
                    labeled_outputs=self.masked_lm_model(**labeled_inputs)
                    
                    labeled_pred_logits=labeled_outputs[0][labeled_batch['mlm_labels']>=0]
                    labeled_pred_logits=F.softmax(labeled_pred_logits,dim=1)  #softmax
            ...
            if self.self_lm_defined_epoch>=0:    
                epoch_iterator = tqdm(train_dataloader, desc="Iteration")
                for i_step, batch in enumerate(epoch_iterator):
                    self.masked_lm_model.train()
                    unlabeled_batch = None
                    lm_unlabeled_batch = None

                    step_cluster_list = self.fnn_cluster_list[i_step*unlabeled_batch_size : (i_step+1)*unlabeled_batch_size if (i_step+1)*unlabeled_batch_size < unlabeled_data_length  else unlabeled_data_length-1]
                    print("step_cluster_list len :",len(step_cluster_list))
                    ...
                    train_step_inputs = {
                        'unlabeled_batch': unlabeled_batch, 'lm_training': lm_training, 'alpha': alpha, 'step_cluster_list':step_cluster_list,
                        "lm_unlabeled_batch":lm_unlabeled_batch, 'use_logits': use_logits, 'temperature': temperature
                    }
              
                    loss=None
                    if loss is None:
                        loss = self.lm_train_step(batch, **train_step_inputs)
                    ...
                    if (step + 1) % gradient_accumulation_steps == 0:
                        torch.nn.utils.clip_grad_norm_(
                            self.masked_lm_model.parameters(), max_grad_norm)
                        optimizer.step()
                        scheduler.step()
                        self.masked_lm_model.zero_grad()
                        global_step += 1
            ...
            with torch.no_grad():
                for _, batch in enumerate(unlabeled_pool_iterator):
                    ...
                    unlabeled_batch = {k: t.to(device) for k, t in batch.items()}
                    unlabeled_inputs=self.generate_default_inputs(unlabeled_batch)
                    unlabeled_outputs=self.masked_lm_model(**unlabeled_inputs)
                    
                    unlabeled_pred_logits=unlabeled_outputs[0][unlabeled_batch['mlm_labels']>=0].detach().cpu() # logits (batchsize,clusters_size)
                    unlabeled_pred_logits=F.softmax(unlabeled_pred_logits,dim=1)  #softmax
                    
                    # max_idxs=unlabeled_pred_logits.argmax(dim=1).cpu().numpy().tolist()
                    # curr_epoch_fnn_cluster_list.extend(max_idxs)
                    

                    if self.vocab_logits_pool==None:
                        self.vocab_logits_pool=unlabeled_pred_logits
                        self.true_label_pool=unlabeled_batch['labels'].detach().cpu()
                    else:
                        self.vocab_logits_pool=torch.cat([self.vocab_logits_pool,unlabeled_pred_logits],0)
                        self.true_label_pool=torch.cat([self.true_label_pool,unlabeled_batch['labels'].detach().cpu()],0)
                        
            for vocab_logits in self.vocab_logits_pool:
                K=30
            ...
            similarity_counts=0
            for cluster_id,label_id in self.cluster_match_dict.items():
            ...
            keys_lst=['loss_s_on_vocab_words','loss_s_with_uncertainty','loss_p_labeled','loss_p_unlabeled','loss_kl','faraway_loss','kv_labeled_loss','loss_total']
            for key in keys_lst:
                self.loss_lm_dict[key]=torch.tensor(self.loss_lm_dict[key], device='cpu',dtype=float)
                self.epoch_loss_lm_dict[key].append(torch.mean(self.loss_lm_dict[key]))
                self.loss_lm_dict[key]=[]

            curr_total_loss = self.epoch_loss_lm_dict['loss_total'][self.self_lm_defined_epoch-1]
            if curr_total_loss<loss_min:
                loss_min=curr_total_loss
            else:
                curr_tolerance+=1
    ...
    def write_results(self, total_classes, known_classes, true_label_pool, fnn_cluster_list, top30_for_each_sample_list=None):
        exp="./cls_results/"
        task_name = ""
        if total_classes==4:
            exp+="agnews"
    ...
    def cluster(self,epoch):
        ...
        print("epoch {}    clusters: {}, total samples: {}, total purity: {}/{}={}".format(epoch,len(cluster_dict),counts,purity_counts,counts,purity_counts/counts if counts!=0 else 0))
    ...
    def cal_match_stats(self, unlabeled_batch: Dict[str, torch.Tensor]):
        ...
        unlabeled_fea=unlabeled_outputs[1][-1][unlabeled_batch['mlm_labels']>=0] 
    ...
    def lm_train_step(self, labeled_batch: Dict[str, torch.Tensor] = None,
                       unlabeled_batch: Optional[Dict[str, torch.Tensor]] = None, lm_training: bool = False,
                       lm_unlabeled_batch: Optional[Dict[str, torch.Tensor]] = None, alpha: float = 0, step_cluster_list=[],**_) -> torch.Tensor:
        """Perform a MLM training step."""
        inputs = self.generate_default_inputs(labeled_batch)
        mlm_labels, labels = labeled_batch['mlm_labels'], labeled_batch['labels']
        
        outputs = self.masked_lm_model(**inputs,output_hidden_states=True) 
        prediction_scores_2=outputs[0][mlm_labels >= 0]  #logits        
        ...
        loss_s_on_vocab_words = nn.CrossEntropyLoss()(prediction_scores_2.view(-1, outputs[0].shape[2]),labels_on_vab.view(-1))
        print("loss_s_on_vocab_words.item() is :",loss_s_on_vocab_words.item())
        
        unlabeled_inputs=self.generate_default_inputs(unlabeled_batch)             
        unlabeled_outputs=self.masked_lm_model(**unlabeled_inputs,output_hidden_states=True)   
        ...
        if step_cluster_list == None:
            print("step_cluster_list is None")
        cluster_list=step_cluster_list 
        new_cluster_list=[]
        ...
        if len(new_cluster_list)!=0:
            ...
            prediction_scores_tmp=unlabeled_outputs[0][unlabeled_batch['mlm_labels']>=0][new_cluster_list]
            ...
            for new_cluster_tensor in prediction_scores_tmp:
                ...
                faraway_labels_loss += nn.CrossEntropyLoss()(all_tensors.view(-1, outputs[0].shape[2]),self.labeled_semantics_pool.view(-1, outputs[0].shape[2]))
            faraway_labels_loss/=len(prediction_scores_tmp)
        else :
            faraway_labels_loss=0
        faraway_labels_loss = 10-faraway_labels_loss
        
        loss = loss_s_on_vocab_words + faraway_labels_loss
        
        self.loss_lm_dict['loss_s_on_vocab_words'].append(loss_s_on_vocab_words.item())
        self.loss_lm_dict['loss_s_with_uncertainty'].append(0)
        self.loss_lm_dict['loss_p_labeled'].append(0)
        self.loss_lm_dict['loss_p_unlabeled'].append(0)
        self.loss_lm_dict['loss_kl'].append(0)
        self.loss_lm_dict['loss_total'].append(loss.item())
        
        self.loss_lm_dict['faraway_loss'].append(faraway_labels_loss)
        self.loss_lm_dict['kv_labeled_loss'].append(0)
    ...
    def mlm_train_step(self, labeled_batch: Dict[str, torch.Tensor],
        ...
        labeled_len=len(labels)
        cpu_labels=labels.to("cpu")
        pos_pairs=[]
        for i in range(labeled_len):
            target_i=cpu_labels[i]
            idxs=np.where(cpu_labels==target_i)[0]
            if len(idxs)==1:
        ...
        labeled_pos_prob =labeled_pred_logits[pos_pairs]
        pos_sim = torch.bmm(labeled_pred_logits.view(labeled_len, 1, -1),
                            labeled_pos_prob.view(labeled_len,-1, 1)).squeeze()
        ones = torch.ones_like(pos_sim)
        bce = nn.BCELoss()
        # pairwise loss
        loss_p_labeled = bce(pos_sim, ones) 
        ...
        print("self.masked_embedding_pool.t().shape ",self.masked_embedding_pool.t().shape)
        ...
        loss_p_unlabeled = bce(pos_sim, ones) 
         ...
         # loss uncertainty 
        curr_uncertainty=self.cal_uncertainty_gpu(unlabeled_pred_logits)
        ...
        if self.limited_clusters > self.total_classes: 
            loss = loss_s_with_uncertainty + loss_p_unlabeled  + loss_p_labeled + loss_kl  
        else :
            loss = loss_s_with_uncertainty + loss_p_unlabeled  + 2*loss_kl  
        
        self.loss_dict['loss_s_on_vocab_words'].append(0)
        self.loss_dict['loss_s_with_uncertainty'].append(loss_s_with_uncertainty.item())
        self.loss_dict['loss_p_labeled'].append(loss_p_labeled.item())
        ...
        self.loss_dict['faraway_loss'].append(0)
