...
def averge_func(lst,epoch):
    res=[]
    sep=len(lst)//epoch
    for i in range(epoch):
        res.append(sum(lst[sep*i:sep*(i+1)])/sep)
    return res
    


def draw_loss_curve_epoch(loss_dict,name,epoch):
    
    
    epoch=len(loss_dict['loss_total'])
    plt.figure(figsize=(24,16))
    x=[ i+1 for i in range(epoch) ]

    ax1=plt.subplot(241)
    loss_s = loss_dict['loss_s_on_vocab_words']
    # loss_s = averge_func(loss_dict['loss_s_on_vocab_words'],epoch)
    if len(loss_dict['loss_s_on_vocab_words'])!=0:
        ax1.plot(x, loss_s, 'g', label='loss_s_on_vocab_words') 
        ax1.set_xlabel('step') 
        ax1.set_ylabel('loss')  
    ...
    ax2=plt.subplot(242)
    loss_s_with_uncertainty = loss_dict['loss_s_with_uncertainty']
    # loss_s_with_uncertainty = averge_func(loss_dict['loss_s_with_uncertainty'],epoch)
    if len(loss_dict['loss_s_with_uncertainty'])!=0:
        ax2.plot(x, loss_s_with_uncertainty, 'b', label='loss_s_with_uncertainty') 
        ax2.set_xlabel('step') 
        ax2.set_ylabel('loss')  
        ax2.legend(loc='upper right')   

    ax3=plt.subplot(243)
    loss_on_labeled = loss_dict['loss_p_labeled']
    # loss_on_labeled = averge_func(loss_dict['loss_p_labeled'],epoch)
    if len(loss_dict['loss_p_labeled'])!=0:
        ax3.plot(x, loss_on_labeled, 'y', label='loss_p_labeled')
        ax3.set_xlabel('step') 
        ax3.set_ylabel('loss')  
        ax3.legend(loc='upper right')   

    ax4=plt.subplot(244)
    loss_p = loss_dict['loss_p_unlabeled']
    # loss_p = averge_func(loss_dict['loss_p_unlabeled'],epoch)
    ax4.plot(x, loss_p, 'c', label='loss_p_unlabeled')
    ax4.set_xlabel('step') 
    ax4.set_ylabel('loss')  
    ...
    # loss_kl = averge_func(loss_dict['loss_kl'],epoch)
    ax5.plot(x, loss_kl, 'm', label="loss_kl")
    ax5.set_xlabel('step') 
    ax5.set_ylabel('loss')  
    ...
    ax6=plt.subplot(246)
    # faraway_loss = averge_func(loss_dict['faraway_loss'],epoch)
    faraway_loss = loss_dict['faraway_loss']
    faraway_loss=torch.tensor(faraway_loss, device='cpu')
    ax6.plot(x, faraway_loss, 'm', label="faraway_loss")
    ax6.set_xlabel('step') 
    ax6.set_ylabel('loss')  
    ax6.legend(loc='upper right')
    
    ax7=plt.subplot(247)
    # kv_labeled_loss = averge_func(loss_dict['kv_labeled_loss'],epoch)
    kv_labeled_loss = loss_dict['kv_labeled_loss']
    ax7.plot(x, kv_labeled_loss, 'm', label="unused")
    ax7.set_xlabel('step') 
    ax7.set_ylabel('loss')  
    ...
    # loss_total = averge_func(loss_dict['loss_total'],epoch)
    ax8.plot(x, loss_total, 'r', label="loss_total")
    ax8.set_xlabel('step') 
    ax8.set_ylabel('loss')  
    ...
    plt.legend() 
    plt.savefig('./pics/pet_train_100_and_test_150/'+name, dpi=300)


def draw_loss_curve(loss_dict,name):
    plt.figure(figsize=(24,16))
    x=[ i+1 for i in range(len(loss_dict['loss_total'])) ]
    ...
    ax6=plt.subplot(236)
    loss_total = loss_dict['loss_total']
    ax6.plot(x, loss_total, 'r', label="loss_total")
    ax6.set_xlabel('step') 
    ax6.set_ylabel('loss')  
    ax6.legend(loc='upper right')   
    ...
    for pattern_id in pattern_ids:
        for iteration in range(repetitions):
            ...
            if do_train:
                ...
                # wrapper.loss
                # name=str(pattern_id)+"-"+str(iteration)+".jpg"
                epoch_unit_name=wrapper.pic_name+"-cluster.jpg"
                lm_epoch_unit_name=wrapper.pic_name+"-lm.jpg"
                # draw_loss_curve(wrapper.loss_dict,name)
                draw_loss_curve_epoch(wrapper.epoch_loss_dict,epoch_unit_name,int(train_config.num_train_epochs))
                draw_loss_curve_epoch(wrapper.epoch_loss_lm_dict,lm_epoch_unit_name,int(train_config.num_train_epochs))
...
def _draw_examples_by_label_probability(examples: List[InputExample], num_examples: int, rng) -> List[InputExample]:
    ...
    return rng.choice(examples, size=num_examples, replace=True, p=label_probabilities).tolist()
