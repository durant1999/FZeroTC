...
def draw_loss_curve(loss_dict,name):
    ...
    # loss_s_on_vocab_words + loss_s_with_uncertainty +loss_p_labeled + loss_p_unlabeled + 5*loss_kl
    ax1=plt.subplot(231)
    loss_s = loss_dict['loss_s_on_vocab_words']
    if len(loss_dict['loss_s_on_vocab_words'])!=0:
        ax1.plot(x, loss_s, 'g', label='loss_s_on_vocab_words') 
    ...
    for pattern_id in pattern_ids:
        for iteration in range(repetitions):
            ...
            if do_train:
                ...
                epoch_unit_name=wrapper.pic_name+".jpg"
...
def train_single_model(model: TransformerModelWrapper, train_data: List[InputExample], config: TrainConfig,
    ...
    device = torch.device(
        config.device if config.device else "cuda:0" if torch.cuda.is_available() else "cpu")
