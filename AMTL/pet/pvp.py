...
class PVP(ABC):
    ...
    def convert_labels_to_whole_vab_labels(self,initial_labels:torch.Tensor)->torch.Tensor:
        label_words= [ self.verbalize(str(label.item()+1))[0] for label in initial_labels]  
        label_words_id=[ get_verbalization_ids(lw, self.wrapper.tokenizer, force_single_token=True) for lw in label_words ]
        # print('AG News World-{} Sports-{} Business-{} Tech-{}'.format(get_verbalization_ids('World', self.wrapper.tokenizer, force_single_token=True),
        #                                                               get_verbalization_ids('Sports', self.wrapper.tokenizer, force_single_token=True),
        #                                                               get_verbalization_ids('Business', self.wrapper.tokenizer, force_single_token=True),
        #                                                               get_verbalization_ids('Tech', self.wrapper.tokenizer, force_single_token=True)))
        # print('labels_words_id :',label_words_id)
        return torch.Tensor(label_words_id)

    def convert_mlm_logits_to_cls_logits_only_seen_class(self, mlm_labels: torch.Tensor, logits: torch.Tensor,seen_classes:int) -> torch.Tensor:
        mask_logits=logits[mlm_labels>=0]
        label_list_seen=self.wrapper.config.label_list[0:seen_classes]
        cls_logits = torch.stack(
            [self.get_some_words_logit(ml,label_list_seen) for ml in mask_logits])
        
        return cls_logits

    def get_some_words_logit(self,logits:torch.Tensor,label_list_seen) -> torch.Tensor:
        label_words_id=[ get_verbalization_ids(lw, self.wrapper.tokenizer, force_single_token=True) for lw in label_list_seen ]
        print(label_words_id)
        return logits[label_words_id]
    ...
    def convert_mlm_logits_to_cls_logits(self, mlm_labels: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        ...
        # print("convert_mlm_logits_to_cls_logits logits size:",logits.shape) #orch.Size([8, 256, 50265])
        # print("convert_mlm_logits_to_cls_logits masked_logits size:",masked_logits.shape) #torch.Size([8, 50265])
        cls_logits = torch.stack(
    ...
    def _convert_single_mlm_logits_to_cls_logits(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        K=30
        top_K_prob_word_indices=torch.topk(logits,K).indices
        # tokenizer.decode([x]) for x in top_values.indices.tolist()
        top_K_prob_words=[self.wrapper.tokenizer.decode([x]) for x in top_K_prob_word_indices.tolist()]
        rank_str=[]
...
class DBPediaPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            ...
            return [self.mask, 'Category:', text_a, text_b], []
        elif self.pattern_id == 2:
            return [text_a, '(', self.mask, ')', text_b], []
        elif self.pattern_id == 3:
            return [text_a, text_b, '(', self.mask, ')'], []
...
class ReutersPVP(PVP):
    VERBALIZER = {
        "1": ["acquisition"],
        "2": ["alum"],
        "3": ["payments","balance"], # bop balance of payments
        "4": ["cocoa"],
        "5": ["coffee"],
        "6": ["copper"],
        "7": ["cotton"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            ...
            return [self.mask, 'Topic:', text_a, text_b], []
        elif self.pattern_id == 2:
            return [text_a, '(', self.mask, ')', text_b], []
        elif self.pattern_id == 3:
            return [text_a, text_b, '(', self.mask, ')'], []
    ...
    def verbalize(self, label) -> List[str]:
        return ReutersPVP.VERBALIZER[label]


class AmazonProductPVP(PVP):
    VERBALIZER = {
        "1":  ["Health","Care"],  
        "2":  ["Home","Kitchen"],  
        "3":  ["Kindle","Store"],     
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            ...
            return [self.mask, 'Product:', text_a, text_b], []
        elif self.pattern_id == 2:
            return [text_a, '(', self.mask, ')', text_b], []
        elif self.pattern_id == 3:
            return [text_a, text_b, '(', self.mask, ')'], []
    ...
    def verbalize(self, label) -> List[str]:
        return AmazonProductPVP.VERBALIZER[label]
...
PVPS = {
    ...
    'yahoo': YahooPVP,
    "amazon-product": AmazonProductPVP,
    "reuters": ReutersPVP,
    'dbpedia': DBPediaPVP,
    'xstance': XStancePVP,
