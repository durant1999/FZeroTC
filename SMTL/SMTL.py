...
def load_pet_configs(args) -> Tuple[WrapperConfig, pet.TrainConfig, pet.EvalConfig]:
    ...
    model_cfg = WrapperConfig(model_type=args.model_type, model_name_or_path=args.model_name_or_path,
                              ...
                              cache_dir=args.cache_dir, known_classes=args.known_classes, limited_clusters=args.limited_clusters, pic_name=args.pic_name, total_classes=args.total_classes, per_known_class_samples_on_labeled_semantics_pool=args.per_known_class_samples_on_labeled_semantics_pool)
...
def load_sequence_classifier_configs(args) -> Tuple[WrapperConfig, pet.TrainConfig, pet.EvalConfig]:
    ...
    model_cfg = WrapperConfig(model_type=args.model_type, model_name_or_path=args.model_name_or_path,
                              ...
                              verbalizer_file=args.verbalizer_file, cache_dir=args.cache_dir, known_classes=args.known_classes, limited_clusters=args.limited_clusters, pic_name=args.pic_name, total_classes=args.total_classes, per_known_class_samples_on_labeled_semantics_pool=args.per_known_class_samples_on_labeled_semantics_pool)
...
def main():
    ...
    parser.add_argument("--known_classes", default=1, type=int,
                        help="known_classes.")
    parser.add_argument("--limited_clusters", default=10, type=int,
                        help="limited_clusters.")
    parser.add_argument("--per_known_class_samples_on_labeled_semantics_pool", default=5, type=int,
                    help=".")
    # total_classes
    parser.add_argument("--total_classes", default=10, type=int,
                        help="total_classes.")
    parser.add_argument("--pic_name", default="default", type=str, required=True,
                        help="The input data dir. Should contain the data files for the task.")
    ...
    # unlabeled_data = load_examples(
    #     args.task_name, args.data_dir, TRAIN_SET, num_examples=args.unlabeled_examples, num_examples_per_label=train_ex_per_label)
    print("unlabeled_data len:",len(unlabeled_data))
    ...
    if args.method == 'pet':
        print("train data size is {}; eval_data size is {}".format(len(train_data),len(eval_data)))
