...
class TransformerModelWrapper:
    ...
    def train(self, task_train_data: List[InputExample], device, per_gpu_train_batch_size: int = 8, n_gpu: int = 1,
        ...
        curr_tolerance=0
        # max_tolerance=2 # for limited
        max_tolerance=3 #for unlimited
        loss_min= 1000000
        pool_size = 3000 
        for _ in train_iterator: #epochs
            ...
            with torch.no_grad():
                ...
                print("pool update finished")
       
            # construct labeled_sample semantics pool
            ...
            if self.self_defined_epoch>=0:    
                ...
                for _, batch in enumerate(epoch_iterator):
                    ...
                    if lm_training:
                        ...
                        # lm_training data
                        # import copy
                        # lm_unlabeled_batch=copy.deepcopy(unlabeled_batch)
                        # lm_input_ids = lm_unlabeled_batch['input_ids']
                        # lm_unlabeled_batch['input_ids'], lm_unlabeled_batch['mlm_labels'] = self._mask_tokens(
                        #     lm_input_ids)
                        # lm_unlabeled_batch = {k: t.to(device)
                        #                    for k, t in lm_unlabeled_batch.items()}
                        lm_unlabeled_batch=None
            ...
            for i in range(self.limited_clusters):
                ...
                # pred_word_list = []
                # if len(self.limited_cluster_dict[i])!=0:
                #     avg_embedding = torch.mean(self.masked_embedding_pool[self.limited_cluster_dict[i]],dim=0)                  
                #     self.vocab_embedding=self.model.decoder.to(torch.device("cpu"))
                #     vocab_logits= self.vocab_embedding(avg_embedding.reshape(-1,768))[0]
                #     K=30
                #     top_K_prob_word_indices=torch.topk(vocab_logits,K).indices
                #     top_K_prob_words=[self.tokenizer.decode([x]) for x in top_K_prob_word_indices.tolist()]
                #     rank_str=[]
                #     for rank,prob_word in enumerate(top_K_prob_words):
                #         rank_str.append((rank+1,prob_word))
                #     print("cluster avg embedding into vocab_logits rank: ",rank_str)
                ...
                if len(pred_word_list)!=0 and i>=self.known_classes: 
                    ...
                    # print("type(tmp_mode) : ",type(tmp_mode))
                    # target_word_list =  self.preprocessor.pvp.verbalize(str(tmp_mode.item()+1))
                    # seamantic_eval(target_word_list, pred_word_list, top_num=1)
                    # seamantic_eval(target_word_list, pred_word_list, top_num=3)
                    # seamantic_eval(target_word_list, pred_word_list, top_num=5)
                    # print("\n\n")
                    
            
            # acc on seen classes:
            print("========Epoch {} Summary========".format(self.self_defined_epoch))
            print("{} Seen classes acc: {}/{}={:5f}".format(self.known_classes,seen_classes_samples_hits_counts,seen_classes_samples_count,seen_classes_samples_hits_counts/seen_classes_samples_count))    
    ...
    def cluster(self,epoch):
        ...
        for key,set_val in cluster_dict.items():
            ...
            tmp_mode= torch.mode(tmp_true_labels).values
            tmp_mode_count= tmp_true_labels.eq(tmp_mode).sum().item()
            # print("tmp_mode_count:",tmp_mode_count)
    ...
    def mlm_train_step(self, labeled_batch: Dict[str, torch.Tensor],
        ...
        if self.total_classes == 4 and self.limited_clusters == 6: # agnews
            # loss_kl_weights = max(0.00001, round(math.pow(0.85,self.self_defined_epoch-1), 5))
            ...
            # loss_kl_weights = max(0.00001, round(math.pow(0.95,self.self_defined_epoch-1), 5))
            loss_kl_weights = max(0.00001, round(math.pow(0.8,self.self_defined_epoch-1), 5))
            loss_kl = loss_kl_weights*loss_kl*2
        elif self.total_classes == 14 and self.limited_clusters == 21: # dbpedia
            # loss_kl_weights = max(0.00001, round(math.pow(0.96,self.self_defined_epoch-1), 5))
            loss_kl_weights = max(0.00001, round(math.pow(0.8,self.self_defined_epoch-1), 5))
            loss_kl = loss_kl_weights*loss_kl*2
        elif self.total_classes == 24 and self.limited_clusters == 36: # amazon
            # loss_kl_weights = max(0.00001, round(math.pow(0.98,self.self_defined_epoch-1), 5))
            loss_kl_weights = max(0.00001, round(math.pow(0.8,self.self_defined_epoch-1), 5))
            loss_kl = loss_kl_weights*loss_kl*2
        ...
        if self.limited_clusters > self.total_classes:
            loss = loss_s_on_vocab_words  + loss_s_with_uncertainty + loss_p_unlabeled  + loss_p_labeled +  loss_kl + faraway_labels_loss 
        else :
            loss = loss_s_on_vocab_words  + loss_s_with_uncertainty + loss_p_unlabeled  + 2*loss_kl + faraway_labels_loss 
